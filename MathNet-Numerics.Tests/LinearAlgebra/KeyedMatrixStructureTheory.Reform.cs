...
namespace MathNet.Numerics.UnitTests.LinearAlgebraTests.Keyed
{
    ...
    {
        [Theory, Timeout(200)]
        public void CanPermuteRows(KeyedMatrix<T> matrix)
        {
            var m = matrix.Clone();
        ...
        [Theory, Timeout(200)]
        public void CanPermuteColumns(KeyedMatrix<T> matrix)
        {
            var m = matrix.Clone();
            var rnd = new System.Random(0);
        ...
        [Theory, Timeout(200)]
        public void CanInsertRow(KeyedMatrix<T> matrix)
        {
            ...
            // Invalid
            Assert.That(() => matrix.InsertRow(0, default(KeyedVector<T>)), Throws.InstanceOf<ArgumentNullException>());
        ...
        [Theory, Timeout(200)]
        public void CanInsertColumn(KeyedMatrix<T> matrix)
        {
            ...
            // Invalid
            Assert.That(() => matrix.InsertColumn(0, default(KeyedVector<T>)), Throws.InstanceOf<ArgumentNullException>());
        ...
        [Theory, Timeout(200)]
        public void CanAppend(KeyedMatrix<T> left, KeyedMatrix<T> right)
        {
            // IF
            ...
            // Invalid
            Assert.That(() => left.Append(default(KeyedMatrix<T>)), Throws.InstanceOf<ArgumentNullException>());
        }

        [Theory, Timeout(200)]
        public void CanAppendIntoResult(KeyedMatrix<T> left, KeyedMatrix<T> right)
        {
            // IF
            Assume.That(left.RowCount, Is.EqualTo(right.RowCount));
            ...
            // Invalid
            Assert.That(() => left.Append(right, default(KeyedMatrix<T>)), Throws.InstanceOf<ArgumentNullException>());
        ...
        [Theory, Timeout(200)]
        public void CanStack(KeyedMatrix<T> top, KeyedMatrix<T> bottom)
        {
            // IF
            ...
            // Invalid
            Assert.That(() => top.Stack(default(KeyedMatrix<T>)), Throws.InstanceOf<ArgumentNullException>());
        }

        [Theory, Timeout(200)]
        public void CanStackIntoResult(KeyedMatrix<T> top, KeyedMatrix<T> bottom)
        {
            // IF
            Assume.That(top.ColumnCount, Is.EqualTo(bottom.ColumnCount));
            ...
            // Invalid
            Assert.That(() => top.Stack(bottom, default(KeyedMatrix<T>)), Throws.InstanceOf<ArgumentNullException>());
        ...
        [Theory, Timeout(200)]
        public void CanDiagonalStack(KeyedMatrix<T> left, KeyedMatrix<T> right)
        {
            ...
            // Invalid
            Assert.That(() => left.DiagonalStack(default(KeyedMatrix<T>)), Throws.InstanceOf<ArgumentNullException>(), "{0}+{1}->{2}", left.GetType(), right.GetType(), result.GetType());
        }

        [Theory, Timeout(200)]
        public void CanDiagonalStackIntoResult(KeyedMatrix<T> left, KeyedMatrix<T> right)
        {
            ...
            // Invalid
            Assert.That(() => left.DiagonalStack(right, default(KeyedMatrix<T>)), Throws.InstanceOf<ArgumentNullException>());
