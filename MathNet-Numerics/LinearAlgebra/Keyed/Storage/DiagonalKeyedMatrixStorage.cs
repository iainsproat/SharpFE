...
{
    [Serializable]
    public class DiagonalKeyedMatrixStorage<T> : KeyedMatrixStorage<T>
        where T : struct, IEquatable<T>, IFormattable
    {
        ...
        internal DiagonalKeyedMatrixStorage(int rows, int columns, T zero)
            : base(rows, columns)
        {
            _zero = zero;
        ...
        internal DiagonalKeyedMatrixStorage(int rows, int columns, T zero, T[] data)
            : base(rows, columns)
        {
            if (data == null)
            {
        ...
        /// </returns>
        public override bool Equals(KeyedMatrixStorage<T> other)
        {
            var diagonal = other as DiagonalKeyedMatrixStorage<T>;
            if (diagonal == null)
            {
        ...
        {
            var diagonalTarget = target as DiagonalKeyedMatrixStorage<T>;
            ...
            var sparseTarget = target as SparseKeyedCompressedRowMatrixStorage<T>;
        ...
        void CopyToUnchecked(DiagonalKeyedMatrixStorage<T> target)
        {
            //Buffer.BlockCopy(Data, 0, target.Data, 0, Data.Length * System.Runtime.InteropServices.Marshal.SizeOf(typeof(T)));
            Array.Copy(Data, 0, target.Data, 0, Data.Length);
        }

        void CopyToUnchecked(SparseKeyedCompressedRowMatrixStorage<T> target, bool skipClearing)
        {
            if (!skipClearing)
            {
                target.Clear();
        ...
        void CopyToUnchecked(DenseKeyedColumnMajorMatrixStorage<T> target, bool skipClearing)
        {
            if (!skipClearing)
            {
                target.Clear();
        ...
        void CopySubMatrixToUnchecked(DiagonalKeyedMatrixStorage<T> target,
            int sourceRowIndex, int targetRowIndex, int rowCount,
            int sourceColumnIndex, int targetColumnIndex, int columnCount)
        {
        ...
        void CopySubMatrixToUnchecked(SparseKeyedCompressedRowMatrixStorage<T> target,
            int sourceRowIndex, int targetRowIndex, int rowCount,
            int sourceColumnIndex, int targetColumnIndex, int columnCount,
            bool skipClearing)
        {
        ...
        internal override void CopySubRowToUnchecked(KeyedVectorStorage<T> target, int rowIndex,
            int sourceColumnIndex, int targetColumnIndex, int columnCount,
            bool skipClearing = false)
        {
            if (!skipClearing)
        ...
        internal override void CopySubColumnToUnchecked(KeyedVectorStorage<T> target, int columnIndex,
            int sourceRowIndex, int targetRowIndex, int rowCount,
            bool skipClearing = false)
        {
            if (!skipClearing)
