...
{
    [Serializable]
    public class SparseKeyedVectorStorage<T> : KeyedVectorStorage<T>
        where T : struct, IEquatable<T>, IFormattable
    {
        ...
        internal SparseKeyedVectorStorage(int length, T zero = default(T))
            : base(length)
        {
            _zero = zero;
            Indices = new int[0];
        ...
        {
            var sparseTarget = target as SparseKeyedVectorStorage<T>;
        ...
        void CopyToUnchecked(SparseKeyedVectorStorage<T> target)
        {
        ...
        void CopySubVectorToUnchecked(SparseKeyedVectorStorage<T> target,
            int sourceIndex, int targetIndex, int count,
            bool skipClearing)
        {
            var offset = targetIndex - sourceIndex;
