...
namespace SharpFE.Examples.Beam
{
    ...
    public class BeamInFull3DModel
    {
        ...
        public void PitchedRoofPortalFrame()
        {
            ...
            ForceVector force = model.ForceFactory.Create(0, 0, 10,0,0,0); //Add load of 10 Newtons in Z-direction
            model.ApplyForceToNode(force, node3);
            
            IFiniteElementSolver solver = new LinearSolver(model);
            FiniteElementResults results = solver.Solve();
            
            DisplacementVector displacement = results.GetDisplacement(node3);
            
            Assert.AreEqual(0.0, displacement.Y, 0.001);
            Assert.AreNotEqual(0.0, displacement.Z);
            Assert.AreEqual(0.0, displacement.X, 0.001); //Displacment of node in X-direction should be 0
        }
    }
}
